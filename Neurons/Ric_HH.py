...
class Neuron:
    def __init__(self):
        ...
        self.alphaH = lambda v : 0.07 * math.exp(v / 20.0)
        self.betaN = lambda v : 0.125 * math.exp(v / 80.0)
        self.betaM = lambda v : 4.0 * math.exp(v / 18)
    ...
    def stimulation(self):
        ...
        for t in range(1, self.steps):
            ...
            Na = self.GNa * m**3 * h * (self.v[t - 1] - self.VNa)
            L = self.Gl * (self.v[t-1] - self.Vl)
            
            dV = (self.i - K - Na - L) * self.dt / self.Cm
