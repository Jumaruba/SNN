	...
	def drawGraph(self): 
		...
		for i in range(self.Ne + self.Ni) :
			for j in range(self.Ne + self.Ni) : 
				...
				graph.add_edge(i,j, weight = weight_value, label = str(i)+"->"+str(j)) 

		# set the edges between neurons 
		pos = nx.circular_layout(graph) 
		nx.draw_networkx_nodes(graph,pos,  node_size = 200, node_color = colors, with_labels = True) 
		nx.draw_networkx_edges(graph,pos, node_size=200, arrowstyle='->',
		...
		nx.draw_networkx_edge_labels(graph, pos,label_pos=0.3, font_size=7, edge_labels=labels, labels = True)
		plt.show()



nn = CustomNetwork(2,1)
nn.drawGraph()
